...
app.get("/novels/search", async (c) => {
  ...
  const { data, error } = await supabase.from("novels")
    .select("id,title,image,origin,status,rating,views")
...
app.get("/chapters/latest", async (c) => {
  ...
  const { data, count } = await supabase.from("chapters")
    .select("id,chapter_number,title,created_at,novel_id,novels(title,image,origin)", { count: "exact" })
...
app.get("/analytics/pageviews", requireAdmin, async (c) => {
  ...
  const byDay = {};
  (data || []).forEach(v => { const day = v.created_at.slice(0, 10); byDay[day] = (byDay[day] || 0) + 1; });
  const labels = [], values = [];
...
app.get("/admin/dashboard-stats", requireAdmin, async (c) => {
  ...
  const [novels, users, comments, reports, newReg, recentRep, pageViews] = await Promise.all([
    ...
    supabase.from("page_views").select("created_at").gte("created_at", new Date(Date.now() - 7 * 86400000).toISOString())
  ]);

  const byDay = {};
  (pageViews.data || []).forEach(v => { const day = v.created_at.slice(0, 10); byDay[day] = (byDay[day] || 0) + 1; });

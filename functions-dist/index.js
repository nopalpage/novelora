...
var shouldSkipCache = /* @__PURE__ */ __name((res) => {
...
var cache = /* @__PURE__ */ __name((options) => {
  ...
  const addHeader = /* @__PURE__ */ __name((c) => {
  ...
  return /* @__PURE__ */ __name(async function cache2(c, next) {
    let key = c.req.url;
...
var generateDigest = /* @__PURE__ */ __name(async (stream, generator) => {
  if (!stream) {
    return null;
  }
  let result = void 0;
...
var stripWeak = /* @__PURE__ */ __name((tag) => tag.replace(/^W\//, ""), "stripWeak");
...
function initializeGenerator(generator) {
  if (!generator) {
    if (crypto && crypto.subtle) {
      generator = /* @__PURE__ */ __name((body) => crypto.subtle.digest(
        {
          name: "SHA-1"
        },
        body
...
__name(initializeGenerator, "initializeGenerator");
var etag = /* @__PURE__ */ __name((options) => {
  const retainedHeaders = options?.retainedHeaders ?? RETAINED_304_HEADERS;
  const weak = options?.weak ?? false;
  const generator = initializeGenerator(options?.generateDigest);
  return /* @__PURE__ */ __name(async function etag2(c, next) {
...
var _getErrorMessage2 = /* @__PURE__ */ __name((err2) => err2.msg || err2.message || err2.error_description || err2.error || JSON.stringify(err2), "_getErrorMessage");
...
var ok = /* @__PURE__ */ __name((c, data, meta = {}) => c.json({ ok: true, data, ...meta }), "ok");
var err = /* @__PURE__ */ __name((c, msg, status = 400) => c.json({ ok: false, message: msg }, status), "err");
...
app.get(
  ...
  async (c) => {
    ...
    if (sort === "popular")
      q = q.order("total_views", { ascending: false });
    if (sort === "new")
    ...
    if (error)
      return err(c, error.message, 500);
...
app.get("/novels/search", async (c) => {
  ...
  const { data, error } = await supabase.from("novels").select("id,title,slug,cover_url,origin,status,avg_rating,total_views").textSearch("title", q, { type: "websearch" }).limit(20);
  if (error)
    return err(c, error.message, 500);
  return ok(c, data);
});
...
app.put("/novels/:id", requireAdmin, async (c) => {
  ...
  if (!supabase)
    return ok(c, body);
  const { data, error } = await supabase.from("novels").update({ ...body, updated_at: /* @__PURE__ */ new Date() }).eq("id", c.req.param("id")).select().single();
  if (error)
    return err(c, error.message);
  return ok(c, data);
});
...
app.get(
  ...
  async (c) => {
    ...
    if (!supabase)
      return ok(c, []);
    const { data, error } = await supabase.from("chapters").select("id,chapter_num,title,word_count,created_at").eq("novel_id", c.req.param("id")).eq("is_draft", false).order("chapter_num", { ascending: true });
    if (error)
      return err(c, error.message, 500);
    return ok(c, data);
  }
...
app.get("/chapters/latest", async (c) => {
  ...
  if (!supabase)
    return ok(c, []);
  const { data } = await supabase.from("chapters").select("id,chapter_num,title,novel_id,novels(title)").eq("is_draft", false).order("created_at", { ascending: false }).limit(10);
  return ok(c, data || []);
});
app.get(
  "/chapters/:id",
  ...
  async (c) => {
    ...
    if (!supabase)
      return err(c, "Not found", 404);
    const { data, error } = await supabase.from("chapters").select("id,novel_id,chapter_num,title,content,word_count,created_at").eq("id", c.req.param("id")).eq("is_draft", false).single();
    if (error)
      return err(c, "Not found", 404);
...
app.put("/chapters/:id", requireAdmin, async (c) => {
  ...
  if (!supabase)
    return ok(c, body);
  const { data, error } = await supabase.from("chapters").update({ ...body, updated_at: /* @__PURE__ */ new Date() }).eq("id", c.req.param("id")).select().single();
  if (error)
    return err(c, error.message);
  return ok(c, data);
});
...
app.get(
  ...
  async (c) => {
    const supabase = db(c);
    if (!supabase)
      return ok(c, MOCK_NOVELS.slice(0, 10));
...
app.get("/analytics/summary", requireAdmin, async (c) => {
  const supabase = db(c);
  if (!supabase)
    return ok(c, MOCK_ANALYTICS_SUMMARY);
...
app.get("/analytics/top-novels", requireAdmin, async (c) => {
  const supabase = db(c);
  if (!supabase)
    return ok(c, MOCK_NOVELS.slice(0, 5));
...
app.get("/me/progress/:novelId", requireAuth, async (c) => {
  ...
  if (!supabase)
    return ok(c, null);
  const { data } = await supabase.from("reading_progress").select("chapter_id, position, chapters(chapter_num, title, novel_id)").eq("user_id", c.get("user").id).eq("chapters.novel_id", c.req.param("novelId")).order("updated_at", { ascending: false }).limit(1).single();
  return ok(c, data);
});
...
app.post("/auth/login", async (c) => {
  ...
  const supabase = db(c);
  if (!supabase)
    return ok(c, { user: { email }, mock: true });
  const { data, error } = await supabase.auth.signInWithPassword({
    email,
    password
  });
  if (error)
    return err(c, error.message, 401);
  return ok(c, { user: data.user, session: data.session });
});
app.post("/auth/register", async (c) => {
  const { email, password, username } = await c.req.json();
  if (!email || !password || !username)
    return err(c, "All fields required");
  const supabase = db(c);
  if (!supabase)
    return ok(c, { user: { email }, mock: true });
  ...
  if (error)
    return err(c, error.message, 400);
  return ok(c, { user: data.user });
});
app.post("/auth/send-verification", async (c) => {
  const { email } = await c.req.json();
  if (!email)
    return err(c, "Email diperlukan");
  const supabase = db(c);
  if (!supabase)
    return ok(c, { sent: true, mock: true });
  const code = Math.floor(1e5 + Math.random() * 9e5).toString();
  const expiresAt2 = new Date(Date.now() + 10 * 60 * 1e3).toISOString();
  const { error: dbErr } = await supabase.from("verification_codes").upsert(
    { email, code, expires_at: expiresAt2, used: false },
    { onConflict: "email" }
  );
  if (dbErr) {
    await supabase.from("verification_codes").insert({ email, code, expires_at: expiresAt2, used: false });
  }
      ...
      <body style="font-family:-apple-system,sans-serif;background:#f0efea;padding:40px 20px;margin:0;">
        <div style="max-width:420px;margin:0 auto;background:#1a1a18;border-radius:16px;padding:36px;color:#e8e0d4;">
          <div style="text-align:center;margin-bottom:28px;">
            <div style="font-size:36px;margin-bottom:10px;">\u{1F4DA}</div>
...
app.post("/auth/verify-code", async (c) => {
  ...
  if (!email || !code)
    return err(c, "Email dan kode diperlukan");
  const supabase = db(c);
  if (!supabase)
    return ok(c, { verified: true, mock: true });
  const { data, error } = await supabase.from("verification_codes").select("id").eq("email", email).eq("code", code).eq("used", false).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).order("created_at", { ascending: false }).limit(1).single();
  if (error || !data)
...
app.get(
  "/health",
  (c) => c.json({ ok: true, ts: Date.now(), runtime: "cloudflare-pages-functions" })
);
...
var MOCK_ANALYTICS_SUMMARY = [
  { icon: "\u{1F441}", label: "Total Page Views", value: "284K", delta: 12, up: true },
  { icon: "\u{1F4DA}", label: "Active Novels", value: "12.4K", delta: 3, up: true },
  { icon: "\u{1F465}", label: "Users", value: "47.8K", delta: 8, up: true },
  { icon: "\u{1F4D6}", label: "Chapters", value: "84K", delta: 5, up: true }
];
var MOCK_CHART_LABELS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
var MOCK_CHART_VALUES = [12400, 14200, 11800, 15600, 17200, 21400, 18900];

// ../.wrangler/tmp/pages-nrVfzO/functionsRoutes-0.9793142571147945.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",

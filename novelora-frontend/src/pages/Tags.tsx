  ...
  return (
    <main className="flex-grow max-w-[1600px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-2 mb-6">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-blue-600 dark:text-blue-500">Tag</h2>
        <h2 className="text-2xl sm:text-3xl font-extrabold text-gray-800 dark:text-white transition-colors duration-300">List</h2>
      </div>

      {/* Top Ad Banner - Hide if no tags */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {isLoading 
          ...
          : allTags.map((tag) => (
              <div key={tag.name} className="flex justify-between items-center bg-gray-100 dark:bg-gray-800/80 px-4 py-2 rounded-lg border border-gray-200/50 dark:border-gray-700 hover:border-blue-500/50 hover:shadow-sm dark:hover:shadow-blue-500/10 transition-all cursor-pointer group">
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate mr-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{tag.name}</span>
                <span className="text-xs text-gray-400 dark:text-gray-500 font-mono shrink-0">{tag.count}</span>
              </div>
            ))}
      </div>

...
export function AdsterraBanner({ 
  ...
  if (!resolvedZoneId) {
    ...
    // Fallback for 300x600 (which wasn't explicitly provided, use 160x600 or 300x250, we'll use 160x600 key but 300x250 might fit better in width, let's use 2a9fe5ef9cc2e976f1faa49c2295e6af)
  ...
  useEffect(() => {
    if (resolvedZoneId && adRef.current && !adRef.current.firstChild) {
      ...
      scriptInvoke.src = `https://commendtwisted.com/${resolvedZoneId}/invoke.js`;
      adRef.current.appendChild(scriptInvoke);
    }
  }, [resolvedZoneId, width, height, format]);
